...
uses
  ...
  jpeg, Grids, DBGridEh, SpeedForm, DBLookupEh, CheckLst, GridsEh,
  DBGridEhGrouping;
...
temp.Position:=poOwnerFormCenter;
 temp. custQuery.Close;
  temp. custQuery.ParamByName('name').AsString:= AnsiUpperCase(clientname.Text+'%');
  temp.custQuery.Open;
  temp.custQuery.Filter:='id='+inttostr(clientid);
  temp.custQuery.Filtered:=true;
  //  temp.Show;
cid:=temp.ShowModal;
if cid >200
  then begin
...
temp.Visible:=false;
temp.Position:=poOwnerFormCenter;
temp.tovarquery.Close;
      temp. tovarQuery.ParamByName('name').AsString:=tovnameedit.Text+'%';
       temp.tovarQuery.ParamByName('tip').AsString:='%';
  if tovarid<>0 then
     begin
     temp.tovarQuery.Filter:='id='+IntToStr(tovarid);
     temp.tovarQuery.Filtered:=true;
     end else temp.tovarQuery.Filtered:=false;

        temp.tovarquery.Open;
...
temp:=Ttovarform.Create(tform(self));
  temp.tovarquery.Close;
  temp.tovarQuery.ParamByName('name').AsString:='%';
  temp.tovarQuery.ParamByName('tip').AsString:='%';
   temp.tovarQuery.ParamByName('sn').AsString:='%';
   temp.tovarquery.Open;
   temp.tovarQuery.Locate('id',podmenaid,[]);
temp.FormStyle:=fsNormal;
temp.Visible:=false;
temp.Position:=poOwnerFormCenter;
tid:=temp.ShowModal;
           ...
           mtConfirmation, [mbYes, mbNo], 0) = mrYes then
            begin
              if not updateRemont then exit;
               dm.newEvent(tovarid,magid,userid,date,'редактирование');
                ...
                if klientchek.Checked
                       then
                           begin
                             ...
                             dataselledit.Value:=sellQuerydata.AsDateTime;
                             enddate.Value:=sellQuerygarantia.AsDateTime;
                             maglabel.Caption:=sellQuerymag.AsString;
                             sellQuery.Close;
                           end;
 ...
 //---------------------------------------------------------------------------------
  function taktform.updateRemont:boolean;
    begin
     result:=false;
        try
    updRemQuery.ParamByName('tovarid').AsInteger:=tovarid;
    ...
    updRemQuery.ParamByName('complect').AsString:=kompedit.Text;
    updRemQuery.ParamByName('vid').AsString:=videdit.Text;
    updRemQuery.ParamByName('price').AsInteger:=priceED.Value;
...
if dostlabel.Caption<>'' then  dm.setreport4(dm.remontprint,nil,nil,nil,'\zayavka.fr3');
