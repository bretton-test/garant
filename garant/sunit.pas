...
interface
 uses  SysUtils,forms,classes,buttons,BARUNIT,DBGridEh,stdctrls;
...
Tsform(self).Height:=APPLICATION.MainForm.ClientHeight-105;
end else Tsform(self).top:=3;

bOk:=tBitBtn(FindComponent('ok'));
bcancel:=tBitBtn(FindComponent('cancel'));
if (bok<>nil) and (bcancel<>nil) then
 ...
 if bcancel<>nil then
  ...
  if bok<>nil then
   begin
     bok.Left:=Tform(self).Width-bok.Width-15;
    end;
